...
export default function MeetingItem({ meeting }) {
    ...
    return <div className="border border-neutral rounded grid grid-cols-3">
		...
		<div className="py-2 px-3 bg-base-200">
			<div>
				Meeting Date: <span className="font-bold text-base-content">{convertDateString(meeting.meetingDate)}</span>
			</div>
			<a href={meeting.zoomLink} target="_blank" className="btn btn-outline btn-xs btn-info normal-case">Open Zoom Link</a>
		</div>
	</div>
}
